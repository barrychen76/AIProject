...
def smart_scale_to_image_2(array, target_size=(64, 64)):
    """
    智能缩放一维数组到指定尺寸的灰度图像（优化版）
    特征改进：
    - 增强数据规范化：自动处理零值和异常数据
    - 改进插值算法：使用三次样条插值保持数据特征
    - 安全边界检查：防止数组越界
    - 自适应亮度优化：自动对比度增强
    
    参数：
    array : 输入的一维数据数组
    target_size : 目标图像尺寸 (宽, 高)，默认(64, 64)
    ...
    print(f"\n[DEBUG] 输入数据统计: len={len(array)}, min={np.nanmin(array):.2f}, max={np.nanmax(array):.2f}")
    ...
    # 改进的标准化逻辑
    # 所有数据不能为0
    zero_values = array[array == 0]
    if len(zero_values) == 0:
        raise ValueError("所有数据不能为0")
        
    # 第一个数据是100，后面的按比例缩放
    scaled_data = np.asarray(array * 100 / array[0], dtype=np.float)
    ...
    min_max_range = max(max_val - min_val, 1e-6)  # 确保最小范围
    
    # 计算原始宽高比（数据长度/数据范围）
    original_aspect = data_length / min_max_range
    ...
    x_scaled = np.linspace(0, 1, scaled_width * 4)  # 4倍超采样
    try:
        try:
            spline = UnivariateSpline(x_original, scaled_data, s=data_length*0.1)
            interpolated = spline(x_scaled)
        except Exception as e:
            raise RuntimeError(f"三次样条插值失败: {str(e)}") from e
    except Exception as e:
        raise RuntimeError(f"三次样条插值失败: {str(e)}") from e
    
    # 生成缩放图像并确保数据类型
    scaled_image = np.clip(interpolated, 0, 255).astype(np.uint8)
    scaled_image = scaled_image.reshape(-1, 1)  # 转换为2D数组
    # 安全创建画布并填充
    try:
        canvas = np.zeros(target_size, dtype=np.uint8)
        # 计算安全填充位置
        pad_left = max((target_width - scaled_image.shape[1]) // 2, 0)
        pad_top = max((target_height - scaled_image.shape[0]) // 2, 0)
        
        # 计算可填充区域
        y_start = pad_top
        y_end = min(pad_top + scaled_image.shape[0], target_height)
        x_start = pad_left
        x_end = min(pad_left + scaled_image.shape[1], target_width)
        
        # 执行安全填充
        canvas[y_start:y_end, x_start:x_end] = scaled_image[:y_end-y_start, :x_end-x_start]
    except ValueError as e:
        raise RuntimeError(f"图像填充失败: {str(e)}") from e
    # 转换并返回图像对象
    try:
        image = Image.fromarray(canvas, 'L')
        image.verify()  # 验证图像数据有效性
        return image
    except Exception as e:
        raise RuntimeError(f"生成图像失败: {str(e)}") from e
    ...
    try:
        ...
        for item in data['traning_data']:
            ...
            for i, f in enumerate(dfs):
                img = smart_scale_to_image_2(f['ma20'].values, target_size=(64, 64))
        ...
        img = smart_scale_to_image_2(df['MA20'].values[:1800], target_size=(3600, 1800))
        img.save("Test/smart_scaled_2.png")

        # 每隔200条数据，生成一次图片
        #for i in range(0, len(df), 1000):
        #    img = smart_scale_to_image(df['MA20'].values[i:i+1000])
        #    img.save(f"Test/smart_scaled_{i}.png")
        #    print(f"图像已保存到 Test/smart_scaled_{i}.png")
        
        # 使用收盘价生成图像
        #img = smart_scale_to_image(df['MA20'].values[:100])
        #img.save("Test/smart_scaled.png")
        #print("图像已保存到 Test/smart_scaled.png")
